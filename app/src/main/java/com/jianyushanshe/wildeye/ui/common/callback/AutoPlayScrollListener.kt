...
import com.jianyushanshe.wildeye.extension.screenHeight
...
/**
 * Author:wangjianming
 * Time:2020/11/20 10:02
 * Description:AutoPlayScrollListener
...
class AutoPlayScrollListener(
    ...
    private fun playVideo(recyclerView: RecyclerView?) {
        recyclerView?.run {
            ...
            if (gsyBaseVideoPlayer != null && needPlay) {
                runnable?.let {
                    val tmpPlayer = it.gsvBaseVideoPlayer
    ...
    private inner class PlayRunnable(var gsvBaseVideoPlayer: GSYBaseVideoPlayer?) : Runnable {
        override fun run() {
            var inPosition = false
            if (gsvBaseVideoPlayer != null) {
                val screenPosition = IntArray(2)
                gsvBaseVideoPlayer!!.getLocationOnScreen(screenPosition)
                val halfHeight = gsvBaseVideoPlayer!!.height / 2
                ...
                if (inPosition) {
                    startPlayLogic(gsvBaseVideoPlayer!!, gsvBaseVideoPlayer!!.context)
                }
            }
        }
